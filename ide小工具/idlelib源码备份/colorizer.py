...
import idlelib.colorizer_user_material#改 colorizer_user_material是存储自定义关键字的模块
DEBUG = False
Define_object_set=idlelib.colorizer_user_material.Define_object_set
#这里需要记录一下调用了几次
#keyword.kwlist[len(keyword.kwlist):]=user_keywords if not {*keyword.kwlist}&{*user_keywords} else keyword.kwlist[len(keyword.kwlist):]
...
def make_pat():
    ...
    cmd_kw = r"\b" + any("CMD", idlelib.colorizer_user_material.user_keywords) + r"\b"#改
    match_softkw = (
    ...
    match_cmd=(r"^[ \t]*(?P<MATCH_CMD>("+
               '|'.join(idlelib.colorizer_user_material.user_keywords)+'))'+
               r"\b(?![ \t]*(?:[:,;=^&|@~)\]}]|\b(?:"+
               '|'.join(idlelib.colorizer_user_material.user_keywords)+r")\b))")#改
    ...
    case_cmd_default = (
        ...
        r"(?P<CASE_CMD>("+
        '|'.join(idlelib.colorizer_user_material.user_keywords)+'))'+
        r"[ \t]+(?P<CMD_DEFAULT_UNDERSCORE>_\b)"
    )#改
    ...
    case_match_cmd=(r"^[ \t]*(?P<CASE_CMD2>("+
                    '|'.join(idlelib.colorizer_user_material.user_keywords)+'))'
                    r"\b(?![ \t]*(?:_\b|[:,;=^&|@~)\]}]|\b(?:"+
                    '|'.join(idlelib.colorizer_user_material.user_keywords)+r")\b))")#改
    #print('测试: ',match_softkw,case_softkw_and_pattern)
    builtinlist = [str(name) for name in dir(builtins)
                   if not name.startswith('_') and
                   name not in keyword.kwlist]
    #print('测试: ',builtinlist)
...
class ColorDelegator(Delegator):
    ...
    def config_colors(self):
        ...
        #print('测试:',self.tagdefs)
        #print('测试:',self.tag_configure,self.tag_configure.__class__)
        for tag, cnf in self.tagdefs.items():
            self.tag_configure(tag, **cnf)
        self.tag_raise('sel')
    #获取颜色底色配置self.tagdefs为配置字典
    def LoadTagDefs(self):
        ...
        self.tagdefs = {
            ...
            'CMD':idleConf.GetHighlight(theme, "cmd")#改
            }
    ...
    def _add_tag(self, start, end, head, matched_group_name):
        ...
        """
        #print('测试:',start, end, head,matched_group_name,)
        tag = prog_group_name_to_tag.get(matched_group_name,
                                         matched_group_name)
        #print('测试:',(tag,f"{head}+{start:d}c",f"{head}+{end:d}c"))
        self.tag_add(tag,
                     f"{head}+{start:d}c",
                     f"{head}+{end:d}c")
